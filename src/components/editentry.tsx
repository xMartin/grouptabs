...
export default class EditEntry extends PureComponent<Props> {
  ...
  renderHeader(showSaveButton: boolean): ReactFragment {
    return (
      <div className="header">
        ...
        <h2>
          {this.props.mode === "new" ? "New transaction" : "Edit transaction"}
        </h2>
        {showSaveButton && (
  ...
  renderContent() {
    ...
    if (this.props.mode === "edit" && !this.props.formState) {
      ...
      return (
        <LoadError message={message} onOkClick={this.props.onCloseClick} />
      );
    }
    ...
    return (
      <div className="scene editEntryScene">
        {this.renderHeader(
          !isLoading &&
            !(
              this.props.remoteTabError ||
              (this.props.mode === "edit" && !this.props.formState)
            )
        )}
