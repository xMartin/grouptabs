...
enum LastSyncedStatus {
  NEVER,
  RECENTLY,
  PAST,
}

const SyncStatus: FunctionComponent<Props> = (props) => {
  useRerenderInterval(5000);
  const isOnline = useNavigatorOnLine();

  let syncStatus: LastSyncedStatus = LastSyncedStatus.NEVER;
  if (props.lastSyncedSuccessfully) {
    if (
      new Date().getTime() - new Date(props.lastSyncedSuccessfully).getTime() <
      17000
    ) {
      syncStatus = LastSyncedStatus.RECENTLY;
    } else {
      syncStatus = LastSyncedStatus.PAST;
    }
  }
  ...
  return (
    <div className="sync-status">
      <div className="icon">
        <svg className="cloud" viewBox="0 0 122.88 79.13">
          <g>
        ...
        <span className="status">
          {(() => {
            if (isOnline) {
              if (syncStatus === LastSyncedStatus.RECENTLY) {
                return "✅";
